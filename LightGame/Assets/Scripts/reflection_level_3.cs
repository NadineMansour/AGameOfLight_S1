...
public class reflection_level_3 : MonoBehaviour {
		...
		if (!gameOver) 
		{
			...
			if (Physics.Linecast (linePositions [1], linePositions [2], out hit)) 
			{
				...
				{	
					...
					float DistanceX  = -collision.x - linePositions[0].x;//Calculating the distance due to the reflection
					DistanceX = linePositions[0].x + (2*DistanceX);
					linePositions[2] = new Vector3 (DistanceX,-3,0);
				}
			}
		}
	}
	...
	void PointRotator()
	{
		...
		float mx = (midpointToRotate.x - pivotPoint.x);
		float my = (midpointToRotate.y - pivotPoint.y);
